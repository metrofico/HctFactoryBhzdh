...
public class MsgMgr extends MsAbstractMsgMgr {
   ...
   private void registerSanYiVoiceReceiver(Context var1) {
      ...
      SysRxSpeechReceiver var3 = new SysRxSpeechReceiver(this.mContext, var2);
      IntentFilter var4 = new IntentFilter();
      var4.addAction("com.hzbhd.speech.to.sys");
      var1.registerReceiver(var3, var4);
   }

   private void sendAirInfoToSpeech(byte[] var1) {
      sa.sendTemp(var1[11] / 2);
      int var2 = DataHandleUtils.getIntFromByteWithBit(var1[8], 0, 3);
      if (var2 != 1) {
         if (var2 != 2) {
            if (var2 != 3) {
               if (var2 == 4) {
      ...
      if (DataHandleUtils.getIntFromByteWithBit(var1[8], 0, 3) == 5) {
         sa.sendDefrost(true);
      } else {
         sa.sendDefrost(false);
      }

      sa.sendWind(DataHandleUtils.getIntFromByteWithBit(var1[8], 3, 4));
      ...
      if (DataHandleUtils.getIntFromByteWithBit(var1[7], 3, 1) == 0) {
         sa.sendAir(false);
      } else {
         sa.sendAir(true);
      }

      if (DataHandleUtils.getIntFromByteWithBit(var1[7], 7, 1) == 0) {
         sa.sendLoop(true);
      } else {
         sa.sendLoop(false);
      }
