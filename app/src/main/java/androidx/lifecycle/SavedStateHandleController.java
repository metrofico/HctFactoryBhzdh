...
final class SavedStateHandleController implements LifecycleEventObserver {
   ...
   private static void tryToAddRecreator(SavedStateRegistry var0, Lifecycle var1) {
      ...
      if (var2 != Lifecycle.State.INITIALIZED && !var2.isAtLeast(Lifecycle.State.STARTED)) {
         ...
         var0.runOnNextRecreation(OnRecreation.class);
      }
   ...
   static final class OnRecreation implements SavedStateRegistry.AutoRecreated {
      public void onRecreated(SavedStateRegistryOwner var1) {
         if (!(var1 instanceof ViewModelStoreOwner)) {
            ...
            SavedStateRegistry var4 = var1.getSavedStateRegistry();
            Iterator var3 = var2.keys().iterator();

            while(var3.hasNext()) {
               SavedStateHandleController.attachHandleIfNeeded(var2.get((String)var3.next()), var4, var1.getLifecycle());
            }

            if (!var2.keys().isEmpty()) {
               var4.runOnNextRecreation(OnRecreation.class);
            }
